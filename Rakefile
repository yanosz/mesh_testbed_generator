...
LEDE_VERSION="18.06.5"
PLATFORM="ar71xx"
PLATFORM_TYPE="tiny"

DOWNLOAD_BASE="https://downloads.openwrt.org/releases/#{LEDE_VERSION}/targets/#{PLATFORM}/#{PLATFORM_TYPE}/"
SDK_BASE="openwrt-imagebuilder-#{LEDE_VERSION}-#{PLATFORM}-#{PLATFORM_TYPE}.Linux-x86_64"
...
def generate_firmware(node_name,profile,packages)
  ...
  FileUtils.mv(
    "#{SDK_BASE}/bin/targets/#{PLATFORM}/#{PLATFORM_TYPE}/openwrt-#{LEDE_VERSION}-#{PLATFORM}-#{PLATFORM_TYPE}-#{profile}-squashfs-sysupgrade.bin",
    "bin/#{node_name}-sysupgrade.bin")
  FileUtils.mv(
    "#{SDK_BASE}/bin/targets/#{PLATFORM}/#{PLATFORM_TYPE}/openwrt-#{LEDE_VERSION}-#{PLATFORM}-#{PLATFORM_TYPE}-#{profile}-squashfs-factory.bin",
